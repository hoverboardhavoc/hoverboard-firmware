//! Fault codes and the per-motor fault-latch unit (`specs/sensing-and-safety.md`, "The fault
//! latches"; the stock contracts are the recovered fault_latch.md / p2s_fault_latch.md, with the
//! codes shared with state.md §5). Recovered from the archived implementation
//! (`archive/accumulated-build`, commit `74b7773`) per the spec's provenance section.
//!
//! This is the OFFROAD-variant shape, and per the spec's variant decision it is the ONLY shape:
//! the standard/p2s unit is this unit with `ext_trip` never driven (a strict subset, pinned by
//! the p2s-subset property test below); which boards bind a hardware trip line to `ext_trip` is
//! board/role wiring at integration, not code.
//!
...
mod tests {
    ...
    fn b_min_halfword_counts_as_nonzero() {
        ...
        assert!(
            !f.is_healthy(),
            "B = 0x8000 is nonzero so A==0,B!=0 is UNHEALTHY"
        );
    }
    ...
    /// A reference model of the STANDARD/p2s latch contract (p2s_fault_latch.md): the same unit
    /// with NO external-trip step. Steps: gate (inert when disabled, code not consumed, counter
    /// frozen); consume the one-shot code mailbox (0x11 latches, 0x21 and any other value do
    /// not; always zeroed); the HEALTHY predicate on sign-extended A/B (HEALTHY zeroes the
    /// counter and returns); UNHEALTHY counts below the cap and clamp-latches at the threshold.
    /// Kept deliberately independent of `FaultLatch::tick` (a second implementation of the p2s
    /// contract, not a call into the first) so the subset test compares two derivations.
    #[derive(Clone, Copy)]
    struct P2sRef {
        running_enable: u8,
        fault_code: u8,
    ...
    impl P2sRef {
        ...
        fn tick(&mut self) {
            ...
            let a = self.a_substate;
            let b = self.b_motion;
            let healthy = (a != 0 || b != 0) && (a == RUN_SUBSTATE || b == 0);
            if healthy {
                self.fault_counter = 0;
                return;
            }
    ...
    #[test]
    fn offroad_unit_with_ext_trip_inactive_is_the_p2s_unit() {
        // The spec's variant decision made enforceable: with ext_trip never driven, the offroad
        // unit must be tick-for-tick identical to the p2s contract across representative streams,
        // including every subtle case the recovered vectors pin. One long scripted stream drives
        // both models through: gate-off freeze with a pending code, enable, over-current latch
        // on a co-writer-cleared latch, stall + unknown-code consumes, the B = 0x8000 nonzero
        // case, healthy-reset at a high counter (no latch write), a threshold-crossing
        // clamp+latch (preset 149_999), cap-guard behavior (presets at and below COUNT_CAP),
        // and healthy/unhealthy alternation.
        //
        // Script fields per step: (enable, code, a, b, preset_counter, clear_latch).
        type Step = (u8, u8, i8, i16, Option<u32>, bool);
        let script: std::vec::Vec<Step> = std::vec![
            // Gate off: pending code + unhealthy fields, everything frozen.
            (0, CODE_OVERCURRENT, 0, 0, Some(1234), false),
            (0, CODE_OVERCURRENT, 0, 0, None, false),
            // Enable; the pending code now consumed and latches; healthy motion.
            (1, 0, RUN_SUBSTATE, 100, None, false),
            // Co-writer clears the latch; stall + unknown codes consume without latching.
            (1, CODE_STALL, RUN_SUBSTATE, 100, None, true),
            (1, 0x55, RUN_SUBSTATE, 100, None, false),
            // B = 0x8000 counts as nonzero: A==0, B=min is UNHEALTHY (counts).
            (1, 0, 0, i16::MIN, None, false),
            (1, 0, 0, i16::MIN, None, false),
            // Healthy reset at a high counter, no latch write.
            (1, 0, RUN_SUBSTATE, 500, Some(200_000), false),
            // Threshold crossing: preset 149_999, one unhealthy tick clamp+latches.
            (1, 0, 0, 0, Some(149_999), true),
            // Steady-state clamp above threshold.
            (1, 0, 0, 0, Some(250_000), true),
            // Cap guard: at the cap (increment suppressed) and just below it (wrap check).
            (1, 0, 0, 0, Some(COUNT_CAP), true),
            (1, 0, 0, 0, Some(COUNT_CAP - 1), true),
            // Alternation: unhealthy, healthy (reset), unhealthy again.
            (1, 0, 1, 500, Some(0), true),
            (1, 0, 1, 0, None, false),
            (1, 0, 0, 500, None, false),
            // Gate off again mid-stream: both freeze.
            (0, CODE_STALL, 0, 0, None, false),
            (1, 0, RUN_SUBSTATE, 0, None, false),
        ];

        let mut off = FaultLatch::new(); // ext_trip stays 0 for the entire stream
        let mut p2s = P2sRef::new();
        for (i, &(en, code, a, b, preset, clear)) in script.iter().enumerate() {
            off.running_enable = en;
            p2s.running_enable = en;
            off.fault_code = code;
            p2s.fault_code = code;
            off.a_substate = a;
            p2s.a_substate = a;
            ...
            assert_eq!(off.fault_code, p2s.fault_code, "code at step {i}");
            assert_eq!(off.fault_counter, p2s.fault_counter, "counter at step {i}");
            assert_eq!(off.fault_latch, p2s.fault_latch, "latch at step {i}");
            assert_eq!(off.ext_trip, 0, "ext_trip stays inactive by construction");
        }
    }
    ...
    fn three_steps_can_each_latch_same_tick_idempotent() {
        ...
        assert_eq!(
            f.fault_latch, 1,
            "idempotent: all three paths set the same flag"
        );
        assert_eq!(f.fault_counter, LATCH_THRESHOLD);
    }
}
