//! The top-level mode state machine (`specs/sensing-and-safety.md`, "The mode machine"; the
//! stock contract is the recovered state.md §3/§4). Recovered from the archived implementation
...
pub struct ModeInputs {
    ...
    /// startup-and-calibration condition (e.g. detection or store mount incomplete, a
    /// provisioning walk in progress). Arming itself is THIS machine's MOE decision (the spec's
    /// `armed == any_moe_allowed()` definition), not a separate layer's input.
...
mod tests {
    ...
    fn clean_power_on_off_init_ready_run_in_three_ticks() {
        ...
        assert!(
            o.init.is_none(),
            "bring-up runs on the INIT pass, not the OFF->INIT tick"
        );
    ...
    fn run_to_shutdown_on_fault_a_clears_moe_and_request() {
        ...
        assert!(
            o.shutdown.is_none(),
            "safe-down runs on the SHUTDOWN pass, not the RUN->SHUTDOWN tick"
        );
    ...
    fn fault_raised_then_lowered_re_enters_init() {
        ...
        assert_eq!(
            o.mode,
            Mode::Init,
            "re-enters INIT once all gates read clear"
        );
    ...
    fn re_power_after_non_sticky_stop_re_runs_init() {
        ...
        assert!(
            !m.moe_allowed(0),
            "MOE not left enabled across the OFF dwell"
        );
        ...
        assert!(
            o.init.unwrap().run_bringup,
            "bring-up repeated every power-on"
        );
        assert!(m.moe_allowed(0));
    }
    ...
    fn n_motor_all_moe_gates_set_at_init_and_cleared_on_shutdown() {
        ...
        assert!(
            m.moe_allowed(0) && m.moe_allowed(1),
            "both motors MOE set on the INIT pass"
        );
    ...
    fn integrates_with_fault_latch_as_fault_source() {
        ...
        assert!(
            o.shutdown.is_none(),
            "the SHUTDOWN pass itself runs next tick"
        );
        let o = m.tick(&inp); // SHUTDOWN pass: outputs safed, MOE cleared -> OFF
        assert_eq!(o.mode, Mode::Off);
        assert!(
            o.shutdown.is_some(),
        ...
        // blocks OFF -> INIT no matter how long the power request is held.
        latch.a_substate = 3;
        latch.b_motion = 100;
        for _ in 0..5 {
            latch.tick();
            assert!(
                latch.is_latched(),
                "the latch is one-way; healthy ticks do not clear it"
            );
            let inp = ModeInputs {
                power_request: true,
                fault_a: latch.is_latched(),
            ...
            let o = m.tick(&inp);
            assert_eq!(o.mode, Mode::Off, "latched fault holds OFF -> INIT blocked");
        }
        assert!(!m.moe_allowed(0));
    }
}
