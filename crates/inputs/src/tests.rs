...
fn combo_only_when_both_members_held() {
    ...
    bank.update(0b0001);
    assert!(!combos.evaluate(&bank).power, "combo drops when a member releases");
}
...
fn pad_high_resets_low_run() {
    ...
    let f = pads.field();
    assert_eq!(f, PAD_A_BIT | PAD_B_BIT, "high resets the low run; 1 low does not drop");
}
...
fn pad_field_both_pads_full_rider() {
    ...
    assert_eq!(f, PAD_A_BIT | PAD_B_BIT, "both pads -> 0b11 full rider field");
    assert_eq!(f, 0b11);
}
...
fn iir_lags_with_design_tau() {
    ...
    assert!(frac < 0.7, "filter lags, must not reach the step in one tau");
}
...
fn iir_q_matches_f64_reference() {
    ...
    assert!(max_out_err <= 1, "Q output within 1 LSB of f64 ref, got {}", max_out_err);
    assert!(
        max_state_err < 0.05,
